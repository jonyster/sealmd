...
| `serve --in doc.md [--port N] [--open] [--slack-webhook URL]` | **Live** loopback review server (requires an existing sidecar; prefer `start`). |
...
1. Launch the server **as a background process** so its output reaches you:
   `node <repo>/skills/seal-review/scripts/seal.mjs start --in doc.md --open`
